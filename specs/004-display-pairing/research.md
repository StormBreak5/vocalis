...
## R16. Revogação é eventual, não instantânea — não existe mecanismo de "kick"

**Decision**: `revoke_display_pairing` só marca `revoked_at`; não existe (e esta feature não introduz) nenhum canal para forçar a TV revogada a agir. O efeito só se torna visível para aquela TV na próxima vez que algo reavalia RLS para ela: um reload da página, ou a reconexão do canal Realtime após queda de rede/retomada de aba. Entre o clique em "Revogar" e esse próximo evento, a TV revogada pode continuar exibindo o telão normalmente, e pode até continuar recebendo eventos `postgres_changes` de `queue`/`sessions` — o Realtime avalia a policy no momento em que o canal é assinado, não em cada evento entregue, então uma assinatura já aberta antes da revogação não é automaticamente cortada.

**Rationale**: isto já estava implícito no contract (`contracts/revoke-display-pairing.md`, seção "Efeito") desde que foi escrito, mas nunca tinha sido registrado como decisão de pesquisa nem verificado explicitamente em teste — T047 é a primeira tarefa que efetivamente depende deste comportamento. A causa raiz é estrutural, não uma falha de implementação: (1) este projeto não tem, em lugar nenhum, um canal servidor→cliente de "comando" (o mesmo já vale para `close_session`, que também não empurra nada para participantes — eles só param de ser autorizados na próxima leitura); (2) mesmo que existisse, a própria TV revogada não pode se auto-vigiar: a policy `display_pairings_select_host` só libera `SELECT` para o Host da sessão, então o telão nunca consegue observar a própria linha em `display_pairings` para perceber que foi revogado por conta própria. A fila e o histórico de reconexão que já existem (queda de rede, retomada de aba, `pageshow`) são o mecanismo que eventualmente converge o estado, exatamente como já convergem para o encerramento de sessão.

**Alternatives rejected**: nenhuma foi implementada agora — deliberadamente fora de escopo desta tarefa (instrução explícita). Duas ficam registradas para uma feature futura, se a demora se mostrar um problema real no uso: **Broadcast dedicado de revogação** (um evento Realtime `broadcast`, não `postgres_changes`, que a TV escuta para o próprio `auth.uid()` e reage imediatamente) — resolveria a auto-vigilância sem abrir `display_pairings` para leitura própria, mas é infraestrutura nova, não coberta por nenhum contract desta feature. **Polling curto do lado da TV perguntando "ainda estou pareado?"** — violaria a constitution III (proibição de polling) só para cobrir uma janela que, na prática, já fecha sozinha no próximo reload/reconexão natural.

## R17. Lista de telões pareados: numeração por ordem de pareamento + horário, sem nova coluna nem nova RPC

**Decision**: `DjDisplayPairingPanel` numera cada linha pela posição no array `pairedDisplays` (já ordenado por `paired_at` ascendente pelo hook `useDisplayPairings`) — "Telão 1", "Telão 2" — e mostra o horário do pareamento (`paired_at` formatado como `HH:MM`, mesmo padrão de `DjParticipantsPanel.joinedTime`). Nenhuma coluna nova em `display_pairings`, nenhuma RPC nova: `id` e `paired_at` já eram tudo que `list_paired_displays`/`useDisplayPairings` devolviam.

**Rationale**: com duas ou mais TVs pareadas, `pairedDisplays` não trazia nada que diferenciasse uma linha da outra — `id` é um UUID sem significado para o Host, e a UI mostrava literalmente "Telão"/"Pareado" repetido em cada linha. Um botão "Revogar" ao lado de uma linha indistinguível é um convite a revogar a TV errada. A numeração por ordem de pareamento dá uma referência imediata ("a segunda que eu parei"); o horário dá uma âncora que **não muda** quando outra TV é revogada (a numeração, por ser posicional, pode reindexar se um item no meio da lista sai — o horário de cada linha continua sendo o mesmo fato histórico, então o Host consegue reconciliar "a TV pareada às 21:34" mesmo depois de uma reindexação). Nenhum dos dois exige tocar em `participants` nem em nenhuma policy — `id`/`paired_at` já eram legítimos de ler por FR-009/R15.

**Alternatives rejected**:
- **Fragmento do UUID como identificador estável** (ex.: "Telão #a3f2"): não reindexa nunca, mas troca um problema de identificação por outro — um Host num bar, com pouca luz, sob pressão, não vai memorizar nem comparar hexadecimais confiavelmente; contraria o princípio do projeto de interfaces extremamente simples para esse contexto de uso. O par (número posicional + horário) já cobre o caso de reindexação através do horário, sem esse custo cognitivo.
- **Deixar o Host nomear a TV no momento do pareamento** (ex.: "Salão", "Balcão"): seria a solução mais amigável de fato, mas exige uma coluna nova em `display_pairings` (ou uma tabela auxiliar) e um campo novo na tela de pareamento da TV ou no painel do DJ — mudança de schema e de fluxo que este ponto de atenção pediu explicitamente para evitar. Fica registrado aqui como a melhoria natural se o "mínimo aceitável" se mostrar insuficiente na prática.

## R18. Nota — `.flow` (fila do telão) colapsa em viewport estreito (pré-existente, fora do escopo)

Achado durante a validação de T042 (fase anterior): o layout `.flow` de `display.module.css` — a grade que organiza "Cantando agora"/"Próximo"/fila — assume um viewport largo (TV/projetor) e colapsa para largura efetivamente zero no viewport móvel padrão assim que há conteúdo de fila para desenhar (o estado de fila vazia usa outro layout, sem `.flow`, por isso o problema não aparece nesse caso). Isso é pré-existente à feature 004: a rota `/sala/[code]/display` nunca foi exercitada em nenhum viewport estreito antes desta feature, porque só o Host a abria, sempre a partir de um botão que abre uma nova aba/janela já redimensionada para TV. Todos os testes E2E de telão (`public-room-display.spec.ts`, e agora os desta feature) setam explicitamente `setViewportSize({width:1920, height:1080})` antes de checar conteúdo — é assim que o problema fica invisível em produção e em teste, e também por que só apareceu agora: T042 foi o primeiro teste a abrir a rota do telão sem esse ajuste de viewport enquanto a fila tinha conteúdo. Não há ação prevista para isso nesta feature — a rota é e continua sendo pensada exclusivamente para TV/projetor (Assumption already existente: "o telão pareado exibe exatamente o mesmo conteúdo que o telão do Host exibe hoje"), e nenhum requisito desta feature pede suporte a viewport de celular para `/display`. Registrado aqui só para não ser redescoberto como surpresa por quem tocar em `display.module.css` no futuro.
