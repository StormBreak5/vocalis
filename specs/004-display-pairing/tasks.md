...
- [X] T015 **GATE — SC-003**: executar `npm run test:db` (adiciona `004_display_pairing_privileges.sql` a `APPROVED_PGTAP_FILES` em `scripts/supabase/pgtap.mjs` primeiro); falha aqui bloqueia a entrega da feature, não a escrita de código de aplicação. **Não usar `npx supabase test db <arquivo> --local`** — esse comando usa o executor da CLI (`pg_prove`/psql, statement a statement), diferente do que a CI roda de fato (`run-pgtap-local.mjs`: um cliente `pg` que envia o arquivo inteiro como uma única query multi-statement); validar por um motor e a CI cobrar outro deixou passar despercebido um bug real por uma feature inteira — ver `validation/final-gate.md`
- [X] T016 **GATE — SC-004**: executar `npm run test:db` (adiciona `004_display_pairing_rls.sql` a `APPROVED_PGTAP_FILES`); mesma ressalva do T015 sobre o executor
- [X] T017 Executar `npm run test:db` (adiciona `004_display_pairing_codes.sql` a `APPROVED_PGTAP_FILES`); mesma ressalva do T015 sobre o executor
...
- [X] T048 [P] Auditar WCAG 2.1 AA, foco, contraste e touch targets ≥48px em `DisplayPairingScreen`/`DjDisplayPairingPanel` — `specs/004-display-pairing/validation/accessibility.md`
- [X] T049 [P] Auditar ausência de polling/`setInterval` de servidor (o contador regressivo local do código de pareamento é cosmético, não busca dados) — `specs/004-display-pairing/validation/no-polling.md`
- [X] T050 Executar `npm run lint`, `npm run typecheck` e `npm run build`
- [X] T051 Executar `quickstart.md` passo a passo (validação manual equivalente a SC-001) e registrar em `specs/004-display-pairing/validation/quickstart-manual.md`
- [X] T052 Atualizar `AGENTS.md`: seção 4, acrescentar as **duas** tabelas novas (`private.display_pairing_codes`, `public.display_pairings` — `display_pairing_attempts` foi removida por R13 e nunca chegou a existir em código, não é uma terceira tabela); seção 5.2, acrescentar as cinco RPCs de pareamento (`generate_display_pairing_code`, `redeem_display_pairing_code`, `get_display_session_details`, `list_paired_displays`, `revoke_display_pairing`) **mais `list_active_queue`** — não é uma RPC nova desta feature no sentido de pareamento, mas R15 a converteu de query PostgREST embutida em RPC `SECURITY DEFINER`, e ela passou a ser o caminho de leitura da fila para as três superfícies (Host, participante, telão pareado), não só do telão — à lista de RPCs disponíveis; registrar explicitamente que o telão pareado é um caminho de autorização estritamente somente leitura, sem nenhuma RPC de escrita aceitando essa identidade — AGENTS.md é carregado como contexto em toda sessão de agente, e já foi corrigido uma vez nesta mesma feature por descrever um schema defasado
### Correção pós-implementação — `fill()` não confiável no WebKit (T061)

- [X] T061 Investigar e corrigir a falha de `display-pairing-revoke.spec.ts` em Mobile Safari (achada no primeiro T053): instrumentação direta provou que é um bug de teste, não de produto — `fill()` do Playwright no WebKit às vezes seta o valor nativo do input sem disparar o `onChange` controlado do React de forma confiável (8/8 sucesso com `pressSequentially()` vs. falha em 4 de 7 tentativas com `fill()`). Corrigido em `e2e/helpers/session.ts` (`pairDisplay`) e `e2e/display-pairing-host-tv.spec.ts` (as duas ocorrências). Decisão registrada em `research.md` R19.

- [X] T053 **GATE FINAL**: `supabase db reset --local` → reaplicar `018` → regenerar tipos → `npm run test:db` (todas as suítes pgTAP aprovadas, T015/T016 obrigatoriamente verdes; **não** `npx supabase test db <arquivo> --local` — a CI não usa esse executor, ver nota abaixo) → `npm run test:unit` → `npm run test:e2e` → lint → typecheck → build; registrar autorização em `specs/004-display-pairing/validation/final-gate.md`. **Autorizado em 2026-08-20** — os cinco specs de pareamento passaram 4/4 execuções completas, nos dois navegadores, sem exceção; a instabilidade pré-existente de Mobile Safari (não relacionada a esta feature, arquivos nunca tocados por ela) foi investigada, registrada como problema aberto e separado em `research.md`, e não bloqueia o gate porque o critério de merge do projeto é o job `e2e-chrome` (`e2e-webkit` é `continue-on-error`, decisão anterior a esta feature). **Reaberto uma vez**: a CI falhou no job `database` porque os quatro arquivos pgTAP novos nunca tinham sido adicionados a `APPROVED_PGTAP_FILES` em `scripts/supabase/pgtap.mjs`, e a validação original deste gate usara `npx supabase test db --local` (executor da CLI) em vez de `npm run test:db` (executor real da CI) — divergência que deixou passar despercebida uma quebra real, causada pela própria `018`, em três asserções pgTAP pré-existentes (`003_queue_rls.sql`, `003_sessions_rls.sql`, `003_session_privileges.sql`). Corrigido; detalhes completos em `validation/final-gate.md`.
