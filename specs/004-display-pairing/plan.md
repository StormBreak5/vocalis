...
- `private.display_pairing_attempts` — log de tentativas de resgate, usado só para o rate limit de `redeem_display_pairing_code`. Mesma exposição zero.
...
## Rate limit e anti-sondagem (FR-015)

`redeem_display_pairing_code` conta as tentativas recentes em `private.display_pairing_attempts` **antes** de gravar qualquer linha nova — por `(session_id, auth_user_id)` quando `p_room_code` resolve, por `auth_user_id` sozinho quando não resolve (`session_id` `NULL`). Só grava a tentativa atual quando essa contagem ainda está abaixo de 10 nos últimos 5 minutos; acima disso, recusa com `PAIRING_CODE_INVALID` sem inserir e sem sequer consultar a tabela de códigos. Contar antes de gravar é o que impede que um chamador já bloqueado continue escrevendo uma linha por chamada — a tabela não tem purga, então gravação incondicional a tornaria ela própria um vetor de escrita sem limite. Sala inexistente, código de pareamento inexistente, expirado, já consumido e limite excedido levantam exatamente a mesma exceção — o cliente nunca recebe informação que distinga esses cinco casos, conforme os Edge Cases da spec. Diferente das outras quatro RPCs da feature, esta não usa `SESSION_NOT_FOUND`/`SESSION_NOT_FOUND_OR_FORBIDDEN`: colapsar sala inexistente no mesmo erro do código errado é o que impede varrer códigos de sala sem limite, já que só esta RPC é alcançável por um chamador sem nenhum vínculo prévio com a sessão.
...
1. `getSessionStatusRowByCode(code)` (existente) resolve `code → session_id` via a policy de `sessions` — agora também succeeds para telão pareado, mas ainda falha para visitante desconhecido/sem sessão anônima, que cai direto no passo 3.
...
3. Se qualquer um dos dois passos falhar: renderiza `<DisplayPairingScreen roomCode={code} />` **em vez de** `redirect(roomPath)` — essa é a mudança central do FR-008.
...
2. `private.display_pairing_codes`, `private.display_pairing_attempts`, `public.display_pairings` com suas constraints e índices (ver `data-model.md`).
3. `REVOKE ALL` explícito nas três tabelas para `PUBLIC, anon, authenticated` (defesa em profundidade nas duas tabelas `private`, já fora do schema exposto; grant mínimo de `SELECT` em `display_pairings` só depois da policy).
...
- `supabase/tests/004_display_pairing_privileges.sql` — toda RPC de escrita existente (`create_queue_entry`, `cancel_queue_entry`, `update_queue_status`, `update_session_status`, `close_session`) chamada com identidade de telão pareado é recusada — prova SC-003 varrendo a lista completa de RPCs de escrita, não só as novas.
- `supabase/tests/004_display_pairing_rate_limit.sql` — 10 tentativas com código errado seguidas de recusa indistinguível na 11ª; contagem de tentativas isolada por `(session_id, auth_user_id)`.
...
|---|---|---|
| Tabelas efêmeras (`display_pairing_codes`, `display_pairing_attempts`) no schema `private`, não `public` | Códigos de pareamento e log de tentativas não têm nenhum consumidor client-side legítimo; mantê-los fora do schema exposto por PostgREST é defesa em profundidade sem custo — é estritamente mais simples que criar policies `RESTRICTIVE` de bloqueio total em `public`, como o projeto teve que fazer para `queue` inserts/deletes diretos. | Colocar as duas tabelas em `public` com policies "deny all" — funciona, mas exige duas policies negativas extras só para replicar o que a ausência do schema já garante de graça. |
